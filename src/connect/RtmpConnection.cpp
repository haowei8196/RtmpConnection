...
- (void) Threadfunc
{
    do {
        if (![self run])
        {
            [NSThread sleepForTimeInterval:_sleepms];
            if (_sleepms>30)
                _sleepms = 1;
            _sleepms = _sleepms*2;
        }
...
RtmpConnection::RtmpConnection(ConnectionType type, ConnectionHandle * callback, int uid, std::string& uri)
{
	_callback = callback;
	_uid = uid;
	_type = type;
...
int RtmpConnection::connectRtmp()
{
	...
	int ret = RTMP_SetupURL(_rtmp, (char*)_uri.c_str(), _uid);
	if (!ret)
	{
...
void RtmpConnection::processPackets()
{
	...
	if (packQueue.size() && _minFrameDropTs < packQueue.front().m_nTimeStamp)
	{
		...
		long curTime = session::TimeMillis();
